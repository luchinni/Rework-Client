...
const FormReview = ({ offer }: any) => {
  ...
  useEffect(() => {
    getOfferForHistory(params.id).then((response) => {
  ...
  const handleChange = (e: any) => {
    ...
    switch (name) {
      case "valoration":
        ...
        errors.valoration =
          valorationPattern.test(value) === false
            ? "Solo números enteros son adimitidos."
            : value[0] === "0"
            ? "No puede inicializar con 0."
            : parseInt(value) <= 0
            ? "La valoración tiene que ser entre 1 a 5."
            : parseInt(value) > 5
            ? "La valoración tiene que ser entre 1 a 5."
            : "";
        break;
      case "review_description":
        errors.review_description =
          formu.review_description.length > 200
            ? "Solo se permiten 200 caracteres"
            : formu.review_description.length < 20
            ? "La cantidad mínima de caracteres es 20"
            : "";
        break;
    }
  ...
  const handleSubmit = (e: any) => {
    ...
    newReviewPost(newReview, type).then(() => {
      let form = document.getElementById("form") as HTMLFormElement | null;
      form?.reset();
      if (userLogged.isWorker) {
        let state = "finalized";
        let proposal = currentOffer.proposals?.find(
          (p: any) => p.state === "contract started"
        );
        if (proposal) {
          ...
          Swal.fire("¡Se registró la review!", "", "success").then(
            (response) => {
        ...
        Swal.fire("¡Se registró la review!", "", "success").then((response) => {
          if (response.isConfirmed) {
            navigate("/home");
          }
        });
  ...
  console.log(currentOffer.userClient);
  ...
  return (
    <div className="form_review">
      ...
      <div className="form_divInput">
        <form id="form" onSubmit={(e) => e.preventDefault()}>
          <h2>Comparte tu opinión!</h2>
          <div className="form_valoration">
            <span>Valoración: </span>
            <input
              name="valoration"
              type="number"
              placeholder="puntaje de 1 a 5"
              onChange={handleChange}
            />
            {error.valoration && <p className="danger">{error.valoration}</p>}
          </div>
